...
const CashOrder = () => {
  ...
  const onSubmit = async (data) => {
    setAdd(true);
    const result = await dispatch(createCashOrder({body:data , id: params.cartId}));
    setAdd(false);
...
<label htmlFor="city">المدينة</label>
          ...
          <textarea className='w-full p-4 rounded-lg border border-gray-500' name="" id="address" cols="30" rows="10" {...register("details", { required: true })}></textarea>
        ...
        <button type="submit" className="bg-blue-700 w-1/2 text-white p-4 rounded-lg cursor-pointer mx-auto mt-2 hover:bg-blue-400">
         {add ? <div className="w-10 mx-auto"><Lottie animationData={loading}/></div>: "طلب"} 
          ...
          <Link to={`/order/hawala/${params.cartId}`} className="text-blue-600 hover:text-blue-700">
           دفع عن طريق حوالة
          </Link>
