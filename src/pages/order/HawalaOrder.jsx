...
setAdd(true);
    const result = await dispatch(createHawalaOrder({ data: formData, id: params.cartId }));
setAdd(false);
  ...
  return (
    <div className="flex justify-center items-center p-10 bg-blue-400">
      <form className="bg-white shadow-2xl h-fit flex justify-center items-start flex-col w-full md:w-1/3 gap-4 rounded-lg p-10"
      ...
      >
        ...
        <input
          type="text"
          id="company"
          {...register("hawalaCompany", { required: "الرجاء إدخال اسم شركة الحوالة" })}
          className="w-full p-4 rounded-lg border border-gray-500"
        />
        {errors.hawalaCompany && <span className="text-red-500">{errors.hawalaCompany.message}</span>}

        {/* تحميل الصورة */}
        <label htmlFor="hawala-upload" className="w-full flex flex-col items-center justify-center border-2 border-dashed border-blue-500 rounded-lg p-6 cursor-pointer bg-white text-blue-600">
          <svg className="w-12 h-12 mb-2" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M3 15.75V19a2.25 2.25 0 002.25 2.25h13.5A2.25 2.25 0 0021 19v-3.25M7.5 10.5L12 6l4.5 4.5M12 6v12" />
          </svg>
        ...
        <button type="submit" className="bg-blue-700 w-1/2 text-white p-4 rounded-lg cursor-pointer mx-auto mt-2 hover:bg-blue-400">
         {add ? <div className="w-10 mx-auto"><Lottie animationData={loading}/></div>: "طلب"} 
        </button>

        <div className="mx-auto">
          <Link to={`/order/cash/${params.cartId}`} className="text-blue-600 hover:text-blue-700">
            دفع عن طريق كاش
          </Link>
        </div>
      </form>
