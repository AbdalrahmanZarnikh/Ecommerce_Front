  ...
  return isLoading == "Success" ? (
    <Container>
      ...
      <div className="flex flex-col md:flex-row gap-[30px]">
        {/* image */}
        {data.images?.length  > 0 ? (<ProductImageSlider images={data.images}/>) : (        <div className="md:w-1/2 w-full">
        ...
        <div>
          ...
          {data.priceAfterDiscount ? (
            <div>
              <span className="text-2xl font-bold text-gray-900">{data.priceAfterDiscount} $</span>{" "}
          ...
          <div className="flex-col items-start sm:flex-row sm:gap-0 gap-[20px] flex items-center mb-6">
            <div className="flex items-center ml-4 border border-gray-300 rounded-md overflow-hidden">
                <button className="flex justify-center items-center w-10 h-10 text-gray-600 hover:bg-gray-100" onClick={() => setquantity(quantity + 1)}>+</button>
                <span className="w-10 h-10 flex justify-center items-center border-x border-gray-300">{quantity}</span>
                <button className="flex justify-center items-center w-10 h-10 text-gray-600 hover:bg-gray-100" onClick={() => {if(quantity>1)setquantity(quantity - 1)}}>-</button>
            </div>
            <button className=" flex items-center justify-between gap-2 cursor-pointer text-white px-[16px] py-[8px] bg-blue-600 rounded-lg ml-4 hover:bg-blue-800">
                 <FiShoppingCart size={20}/>اضافة للسلة</button>
                 <button className="p-3 flex items-center justify-center rounded-md border-gray-300 text-gray-600 border hover:border-gray-400"><FaRegHeart size={20}/>
                 </button>
          </div>
                  {/* features */}
        <Feature title="شحن سريع" desc="توصيل خلال 2-5 ايام"><FaTruck className="text-blue-600" /></Feature>
        <Feature title="ضمان جودة" desc="منتج أصلي 100% مع ضمان لمدة سنة"><AiFillSafetyCertificate className="text-blue-600" /></Feature>

        </div>
      </div>
      {/* reviews */}
      <div className="">
        <h1 className="text-gray-500 mb-6">التقييمات ({data.reviews?.length})</h1>
        {data.reviews?.map((review , index) => <div key={index} className="border-b border-gray-200 pb-6">
